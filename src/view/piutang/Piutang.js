...
const Piutang = () => {
    ...
    useEffect(() => {
        ...
        const fetchList = async () => {
            try {
                const buckets = await bucketService.getByType('piutang');
                setBuckets(buckets);
            } catch (error) {
    ...
    return (
        <>
            <div className="position-fixed fixed-bottom fixed-right pe-4 pb-4">
                <a href="/piutang/create" className="btn btn-primary rounded-pill p-3 float-end"><i className='bx bx-plus'></i></a>
            </div>
                ...
                <div className="row animate__animated animate__fadeIn">
                    ...
                    {buckets.map(bucket => (
                        <div key={bucket.id} className="col-12 mb-3">
                            <a href={'/piutang/detail/' + bucket.id}>
